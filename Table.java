...
class Table {
  ...
  private static Map<String, Entry> countryTable = new HashMap<>();  
  ...
  public static void incMedal(String name) {
    // Increment the medal total for country "name" -- if country doesn't exist
    // create it.
  ...
  public static void printTable() {
    ...
    for (Entry entry : countryTable.values()) {
      Out.format("%-15s %d%n", entry.name, entry.total);
    }
  } // printTable
