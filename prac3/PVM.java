...
//Changed lines Question 4: (129, 344-367), Question 5(447-486) - Dean, Alisha, Zaza (Malicious Hadeda)
...
import library.*;
  ...
  // Utilities
    ...
    static final int minInt = Integer.MIN_VALUE; //added minValue for ints
    ...
    // data and results are used for I/O.  Tracing at the code level may be requested
      ...
      do {
        ...
        switch (cpu.ir) {         // execute
          ...
          //Changed code for MultoiplicATION ERROR HANDLING AND DIVISION ERROR HANDLING (case mul, div, rem)
          case PVM.mul:           // integer multiplication
            tos = pop(); 
            sos = pop();          //push(pop() * tos);
            long result = (long) sos * (long) tos; // Use long to detect overflow by performing the operation in a wider range and then checking if the result fits within the 32-bit integer range.
            if (result > maxInt || result < minInt) ps = badVal;
            else push((int) result);
            break;
          case PVM.div:           // integer division (quotient)
            tos = pop();          //push(pop() / tos);
            if (tos == 0) { //checking for division error
              ps = divZero;
            } else {
              push(pop() / tos); //else execute
            }
            break;
